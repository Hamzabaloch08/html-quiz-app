...
function nextQues() {
    ...
    for (let i = 0; i < getOptions.length; i++) {
        if (getOptions[i].checked) {
            ...
            let selectedAnswer = js[index - 1][`option${selectedValue}`]
            let correctOption = js[index - 1][`correctOption`]
    ...
    if (index >= js.length) {
        alert(`${(score / js.length) * 100}%`);
        clearInterval(interval)
    } else {
        ques.innerText = js[index].question
        opt1.innerText = js[index].option1
        opt2.innerText = js[index].option2
        opt3.innerText = js[index].option3
        index++
        time = 6
    }
}
